...
def add_comment(request, a_id):
    ...
    a.comment_set.create(author=request.POST['comment_author'], text=request.POST['comment_text'])  # извлекаем данные из формы
...
def add_article(request):
    Article.objects.create(title=request.POST['article_title'], text=request.POST['article_text'])  # применяем метод .create(...) к объекту Article
